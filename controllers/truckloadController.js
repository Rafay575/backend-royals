...
exports.submitTruckloadQuote = (req, res) => {
    ...
    if (!firstName || !lastName || !phone || !email || !company || !origin || !destination || !pickupDate || !deliveryDate || !commodity || !weight || !equipment || !trailerSize) {
    ...
    pool.query(query, [firstName, lastName, phone, email, title, company, origin, destination, pickupDate, deliveryDate, commodity, weight, equipment, trailerSize, specialInstructions], (err, result) => {
        if (err) {
